...
(function () {
  ...
  /* ---------------------------------------------------------
     Lightbox gallery
     --------------------------------------------------------- */
  /* Built once, then bound to whatever '.gallery__item' elements exist. Callable
     again via window.initLightbox() so content rendered later — a feature-flagged
     experiment, say — joins the same viewer instead of shipping a second one.
     Binding is idempotent: each tile is marked once. */
  var lb = null, lbItems = [], lbCur = 0, lbLastFocus = null;
  ...
  function buildLightbox() {
    ...
    lb.innerHTML =
      ...
      '<div><img alt=""><p class="lightbox__cap"></p>' +
      '<p class="lightbox__src"><a target="_blank" rel="noopener"></a></p></div>';
    document.body.appendChild(lb);

    $('.lightbox__close', lb).addEventListener('click', closeLb);
    $('.lightbox__prev', lb).addEventListener('click', function () { showLb(lbCur - 1); });
    $('.lightbox__next', lb).addEventListener('click', function () { showLb(lbCur + 1); });
    lb.addEventListener('click', function (e) { if (e.target === lb) closeLb(); });

    document.addEventListener('keydown', function (e) {
      if (!lb.classList.contains('is-open')) return;
      if (e.key === 'Escape')     { closeLb(); return; }
      if (e.key === 'ArrowLeft')  { showLb(lbCur - 1); return; }
      if (e.key === 'ArrowRight') { showLb(lbCur + 1); return; }
  ...
  function showLb(i) {
    ...
    $('img', lb).alt = a.getAttribute('data-alt') || '';
    $('.lightbox__cap', lb).textContent = a.getAttribute('data-cap') || '';
    var srcWrap = $('.lightbox__src', lb), link = $('a', srcWrap);
    var href = a.getAttribute('data-link');
    if (href) { link.href = href; link.textContent = a.getAttribute('data-link-label') || 'View source'; }
    srcWrap.style.display = href ? '' : 'none';
    // Only offer prev/next when there is more than one photo.
    var many = lbItems.length > 1;
    $('.lightbox__prev', lb).style.display = many ? '' : 'none';
    $('.lightbox__next', lb).style.display = many ? '' : 'none';
  }
  ...
  function initLightbox() {
    var found = $$('.gallery__item');
    if (!found.length) return;
    buildLightbox();
    lbItems = found;
    found.forEach(function (a, i) {
      if (a.dataset.lbBound) return;
      a.dataset.lbBound = '1';
      a.addEventListener('click', function (e) {
        e.preventDefault();
        openLb(lbItems.indexOf(a) === -1 ? i : lbItems.indexOf(a));
      });
    });
  }
